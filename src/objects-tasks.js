...
function shallowCopy(obj) {
  const source = {};
  const newObj = Object.assign(source, obj);
  return newObj;
}
...
function mergeObjects(objects) {
  ...
  objects.forEach((obj) =>
    Object.entries(obj).forEach(([key, value]) => {
      result[key] = (result[key] || null) + value;
    })
  );
  return result;
}
...
function isEmptyObject(obj) {
  ...
  if (Array.prototype.includes.call(arr, arr[0])) {
    result = false;
  }
  return result;
}
...
function makeWord(lettersObject) {
  const obj = {};
  const entri = Object.entries(lettersObject);
  entri.forEach((e) => {
    const letter = e[0];
    e[1].forEach((num) => {
      obj[num] = letter;
...
function sellTickets(queue) {
  ...
  for (let i = 0; i < queue.length; i += 1) {
    if (queue[i] === 25) {
      ...
      if (money.includes(25)) {
        money.splice(money.indexOf(25), 1);
      } else {
        change = false;
      }
    } else if (queue[i] === 100) {
      if (money.includes(50) && money.includes(25)) {
        money.splice(money.indexOf(50), 1);
        money.splice(money.indexOf(25), 1);
      } else if (money.filter((m) => m === 25).length >= 3) {
        money.splice(money.indexOf(25), 3);
      } else {
        change = false;
      }
    }
...
function sortCitiesArray(arr) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    for (let j = 0; j < arr.length - i - 1; j += 1) {
      if (
        sortArr[j].country > sortArr[j + 1].country &&
        sortArr[j].country !== sortArr[j + 1].country
      ) {
        ...
        sortArr[j].country === sortArr[j + 1].country &&
        sortArr[j].city > sortArr[j + 1].city
      ) {
        const temp = sortArr[j];
        sortArr[j] = sortArr[j + 1];
        sortArr[j + 1] = temp;
